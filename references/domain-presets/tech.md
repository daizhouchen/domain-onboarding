...
> v0.2 sample 严格自检（向量数据库）发现：12 把分析刀里 4 把缺席（Wardley / Perez / Taleb / 网络效应）。这 4 把恰好是技术新兴赛道的 must-have。v0.3 把它们提升为**强制质量门**——漏了任何一把，技术领域 sample 不算合格。
...
| 必用刀 | 段落必含什么 | 关键词触发 |
|-------|------------|-----------|
| **Wardley Map 演化象限** | 一段"把栈拆开看演化坐标——X 处于 genesis / custom / product / commodity 的哪个象限，未来 N 年向右移多远" | 演化象限 / genesis / custom / product / commodity / 向右移 / 商品化 |
| **Carlota Perez 资本周期** | 必含"导入期 / 疯狂期 / 转折点 / 协同期 / 成熟期"判断（不写学者名） | 导入期 / 疯狂期 / 转折点 / 协同期 / 部署期 / 资本周期 |
| **Christensen 创新者窘境** | 必含一段"哪个低端方案在悄悄追上需求曲线"——找出本领域的"低端入侵者" | 低端 / 够用就好 / 颠覆 / 性能曲线 / 需求曲线 / 下沉 |
| **Taleb 反脆弱** | 玩家命门段必含"反脆弱 / 脆弱 / 杠铃 / 稳健"四档评分（见下表 B） | 反脆弱 / 脆弱 / 杠铃 / 黑天鹅 / 尾部 / 稳健 |
...
- [ ] 范式层章节是否含 Wardley 化（**演化象限 / genesis / commodity 至少触发 1 次**）？
- [ ] 范式层章节是否含 Perez 化（**导入期 / 疯狂期 / 转折点 / 协同期 至少触发 1 次**）？
- [ ] 玩家命门段是否含反脆弱评分（**反脆弱 / 杠铃 / 脆弱 / 稳健 四档至少出现 3 档**）？
- [ ] 第 5 章是否显式回答"是否存在网络效应"（**有 = 写明哪一型；无 = 写明零网络效应是结构性的**）？
- [ ] 跨领域同构是否 ≥1 面跨行业（**IT 之外**——历史 / 金融 / 文化 / 制造业任一）？
- [ ] Christensen 低端颠覆段是否点名了具体的"低端入侵者"（不是泛泛说"会被颠覆"）？
